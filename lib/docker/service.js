...
function createDocklabServerProxy(baseUrl, apiKey) {
    ...
    return {
        ...
        async createNetwork(opts) {
            // docklab-server expects lowercase 'name', dockerode uses uppercase 'Name'
            const payload = { name: opts.Name || opts.name }
            return docklabServerJson('/api/networks', { method: 'POST', body: JSON.stringify(payload) })
        },
